...
def cost_analyze():
    ...
    try:
        ...
        result = cost_invoke(validation['data'])
...
def operations_analyze():
    ...
    try:
        ...
        # Validate input
        validation = validate_request(payload, OperationsRequest)
        if not validation['valid']:
            log_with_context(api_logger, logging.WARNING, "Invalid operations request", errors=validation['errors'])
            return jsonify({'success': False, 'error': 'Invalid request', 'details': validation['errors']}), 400
        
        result = ops_invoke(validation['data'])
...
def orchestrate_full_analysis():
    try:
        payload = request.get_json() or {}
        result = orchestrator.orchestrate_full_analysis(payload)
        return jsonify(result)
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 500

@app.route('/orchestrate/smart-architecture', methods=['POST'])
def orchestrate_smart_architecture():
    try:
        payload = request.get_json() or {}
        requirements = payload.get('requirements', {})
        result = orchestrator.orchestrate_smart_architecture_design(requirements)
        return jsonify(result)
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 500
...
if __name__ == '__main__':
    ...
    print("  - POST /orchestrate/smart-architecture")
